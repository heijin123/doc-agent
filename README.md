...
> 上传页「最近上传记录」面板已接好：上传成功即标绿「上传成功」（文件已落地服务端，切片 / 入库后台跑，无需挂着页面），面板加载即拉取 `/jobs` 并每 3 秒轮询非终态任务，刷新页面也能回看历史任务（任务注册表在服务端内存，跨刷新保留）；存在非终态任务时轮询持续到全部终态后自动停止。
...
| — 增量 — | 切片质量缓存（`data/chunk` 落盘）/ 异步摄取（线程池 + job 注册表 + `/status` + `/jobs`）/ Chroma 并发落库加固（单例 client + 落库锁）/ 上传页「最近上传记录」面板 JS 接好（标绿成功 + 轮询回填 + 刷新回看） | ✅（真实链路 4 并发零错误；前端面板已联调） |
